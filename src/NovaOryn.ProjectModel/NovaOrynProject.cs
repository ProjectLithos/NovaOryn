    ...
    string OutputDirectory)
{
    ...
    public bool Validate(out string error)
    {
        ...
        if (!string.Equals(TargetArchitecture, "x64", StringComparison.OrdinalIgnoreCase)) { error = "NovaOryn 0.0.14 supports x64 only."; return false; }
        if (!string.Equals(BootProtocol, "Uefi", StringComparison.OrdinalIgnoreCase)) { error = "NovaOryn 0.0.14 supports UEFI only."; return false; }
