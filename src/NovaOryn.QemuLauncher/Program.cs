...
static int MainEntry(string[] args)
{
    ...
    string[] qemuArguments = BuildArguments(ovmfCode, variableStore, runImage, serialLog);
    ...
    try
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            ...
            if (serialText.Contains("NovaOryn KMain started.", StringComparison.Ordinal) &&
                serialText.Contains("CPU halted.", StringComparison.Ordinal))
            {
                Thread.Sleep(1500);
                if (process.HasExited)
                {
                    int exitCode = process.ExitCode;
                    process.Dispose();
                    return Fail($"QEMU exited after CPU.Halt() output. Exit code: {exitCode}.");
                }
        ...
        TryStop(process);
        return Fail($"Timed out before the managed runtime acceptance text appeared. Serial log: {serialLog}");
    }
...
static string[] BuildArguments(string ovmfCode, string variableStore, string runImage, string serialLog)
{
    return
    [
        "-machine", "q35",
