...
The renderer has one authoritative rendered-size input: `FontSize`. It is the glyph height in framebuffer pixels. The default is 32 pixels.
...
At the default `FontSize` of 32 pixels, the resulting measurements are:

- rendered glyph width: 16 pixels;
- rendered glyph height: 32 pixels;
- character advance: 20 pixels;
- line height: 40 pixels.

Generated freestanding kernels pass an explicit 32-pixel size to `KernelConsole.Initialize(boot, 32U)`. Kernel authors can choose any supported pixel height through the overload, and `KernelConsole.FontSize` reports the exact value the renderer accepted. The reusable `NovaOryn.Console.Framebuffer` assembly accepts the same exact pixel height through `FramebufferConfiguration.FontSize` and reports it through `FramebufferConsole.FontSize`.
...
`docs/assets/NovaOryn-Mono-32px.png` is generated with the same 32-pixel dimensions and nearest-neighbour source sampling used by the default renderer. `docs/assets/NovaOryn-Mono-8x16.png` remains the unscaled embedded raster-master specimen.
