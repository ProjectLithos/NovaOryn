...
Release 0.0.49 corrects the public API audit status message in the documentation generator. The Windows path is now represented by a valid C# verbatim interpolated string, so the generator compiles and writes both the SDK usage site and `Artifacts\Documentation\PublicApiAudit.json`.
...
See `docs/Release-0.0.49.md` for this release.
...
Run `Install-NovaOrynVSIX.bat`, then create a **NovaOryn Kernel 0.0.49** project in Visual Studio. F5 and Ctrl+F5 invoke the NovaOryn build-and-run pipeline.
