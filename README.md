...
Release 0.0.83 removes recognised SDK-generated monolithic kernels without creating backup copies. The external project refresh deletes obsolete root-level `Kernel.cs` files and overwrites generated nested kernels with the clean high-level `Kernel\Kernel.cs`, while preserving genuinely user-authored kernels.
...
See `docs/Release-0.0.83.md` for this release.
...
Run `Install-NovaOrynVSIX.bat`, then create a **NovaOryn Kernel 0.0.83** project in Visual Studio. F5 and Ctrl+F5 invoke the NovaOryn build-and-run pipeline.
...
Version 0.0.83 adds all 256 x64 IDT vectors, a normalised managed/native exception frame, driver-vector allocation, handler lifecycle management, IST-aware essential exception handlers, and terminal fatal handling.


Version 0.0.83 adds controller-neutral IRQ routing, legacy PIC takeover, APIC EOI/IPI, I/O APIC redirection, MSI/MSI-X message creation, x2APIC delivery, affinity, priorities, polarity, trigger mode, and masking.
