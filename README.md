...
Release 0.0.73 fixes external kernel migration for historical projects whose generated monolithic kernel is at the project root as `Kernel.cs`. The refresher backs up and removes that obsolete file, installs the clean high-level `Kernel\Kernel.cs`, and the build verifies that no low-level implementation remains visible in the end-user kernel.
...
See `docs/Release-0.0.73.md` for this release.
...
Run `Install-NovaOrynVSIX.bat`, then create a **NovaOryn Kernel 0.0.73** project in Visual Studio. F5 and Ctrl+F5 invoke the NovaOryn build-and-run pipeline.
...
Version 0.0.73 adds all 256 x64 IDT vectors, a normalised managed/native exception frame, driver-vector allocation, handler lifecycle management, IST-aware essential exception handlers, and terminal fatal handling.


Version 0.0.73 adds controller-neutral IRQ routing, legacy PIC takeover, APIC EOI/IPI, I/O APIC redirection, MSI/MSI-X message creation, x2APIC delivery, affinity, priorities, polarity, trigger mode, and masking.
